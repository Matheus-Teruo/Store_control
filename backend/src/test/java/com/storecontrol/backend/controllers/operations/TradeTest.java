...
public class TradeTest extends BaseTest {
  ...
  void testCreateTradeSuccess() throws Exception {
    ...
    when(authentication.getPrincipal()).thenReturn(mockVoluntary);
    ...
    verifyNoMoreInteractions(voluntaryService);
  }
  ...
  void testReadTradeSuccess() throws Exception {
    ...
    when(voluntaryService.safeTakeVoluntaryByUuid(tradeView.getVoluntaryUuid()))
        .thenReturn(mockVoluntary);
    ...
    verify(voluntaryService, times(1)).safeTakeVoluntaryByUuid(tradeView.getVoluntaryUuid());
    verifyNoMoreInteractions(service);
    verifyNoMoreInteractions(voluntaryService);
  }
